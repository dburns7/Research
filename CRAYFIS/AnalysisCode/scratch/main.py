...
if __name__== "__main__":
  ...
  for i in xrange(len(hyl)):
    ...
    fmb.append(cpID)		#cpID		= 0
    fmb.append(cpx)		#cpx		= 1
    fmb.append(cpy)		#cpy		= 2
    fmb.append(cpval)		#cpval		= 3
    fmb.append(cpavg3)		#cpavg3		= 4
    fmb.append(cpavg5)		#cpavg5		= 5
    fmb.append(cpma)		#cpma		= 6
    fmb.append(ceID)		#ceID		= 7
    fmb.append(cet)		#cet		= 8
    fmb.append(celon)		#celon		= 9
    fmb.append(celat)		#celat		= 10
    fmb.append(cepavg)		#cepavg		= 11
    fmb.append(cepstd)		#cepstd		= 12
    fmb.append(ceorx)		#ceorx		= 13
    fmb.append(ceory)		#ceory		= 14
    fmb.append(ceorz)		#ceorz		= 15
    fmb.append(cxID)		#cxID		= 16
    fmb.append(cxt)		#cxt		= 17
    fmb.append(cxL1)		#cxL1		= 18
    fmb.append(cxL2)		#cxL2		= 19
    fmb.append(cxdrp)		#cxdrp		= 20
    fmb.append(ccounts)		#ccounts	= 21
    fmb.append(clengths)	#clengths 	= 22
  ...
  for i in xrange(len(hyl)):
    strInListRunID=get_run_ID(sys.argv[i+1])[0] #figured it out! []=index
    print listRunID, sum(hylem[i][21]), get_run_length(hyl[i][17]), sum(count_hits(hylem[i][1], hylem[i][2], hylem[i][3], hylem[i][19]))
    #run ID, total run time, total number of pixels
